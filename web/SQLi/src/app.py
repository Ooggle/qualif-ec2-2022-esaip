...
def pictures():
    ...
    if request.method == "POST":
        ...
        # INSERT image in db
        cur.execute(f"INSERT INTO pictures (file_name, userID, file_length, file_content) VALUES ('{file_name}', ?, ?, ?);", (session["userID"], file_length, file_content))
        db.commit()

    pictures = cur.execute(f"SELECT file_name, file_content FROM pictures WHERE userID = ?", (session["userID"],)).fetchall()
    return render_template("pictures.html", len=len(pictures), pictures=pictures)# , logged=True)
...
def login():
    ...
    if request.method == "GET":
        ...
        if username == "" or password == "":
            ...
            if len(verify) != 0:
                session["logged"] = True
                response = redirect("/pictures", 302)
            else:
